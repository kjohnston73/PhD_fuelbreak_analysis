...
M1 <- lm(IAG ~ aspect_card + graze + sand + clay + ELEVATION +
...
# Step 2 - fit the gls
M2 <- gls(IAG ~ aspect_card + graze + sand + clay + ELEVATION +
            imazapic_binary, 
          method = "ML", data = data)
summary(M2)
AIC(M2) # 736.3736
...
M3 <- lmer(IAG ~ aspect_card + graze + sand + clay + ELEVATION +
            imazapic_binary + (1 | plot), REML = TRUE, data = data)
## compare gls to random-intercept lme
anova(M2,M3) # M3 AIC 729.0440 - lower than M2
summary(M3)
plot(M3)
...
M3.full <- lmer(IAG ~ aspect_card + graze + sand + clay + ELEVATION +
                  imazapic_binary + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)

#drop aspect and go again
M3.full <- lmer(IAG ~ graze + sand + clay + ELEVATION +
                  imazapic_binary + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)

#drop elevation and go again
M3.full <- lmer(IAG ~ graze + sand + clay + imazapic_binary + (1 | plot), REML = TRUE, data = data)
drop1(M3.full) 

#drop graze and go again
M3.full <- lmer(IAG ~ sand + clay + imazapic_binary + (1 | plot), REML = TRUE, data = data)
plot(M3.full)
drop1(M3.full)

d1 <- lmer(IAG ~ sand + clay + (1 | plot), REML = TRUE, data = data)
anova(M3.full, d1)

#drop imazapic and go again
M3.full <- lmer(IAG ~ sand + clay + (1 | plot), REML = TRUE, data = data)
plot(M3.full)
drop1(M3.full)

##
r.squaredGLMM(M3.full)
summary(M3.full)
plot(resid(M3.full, type = "deviance"))
