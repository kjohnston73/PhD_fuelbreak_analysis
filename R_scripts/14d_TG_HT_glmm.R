...
M1 <- lm(max_height ~ aspect_card + graze + sand + clay + ELEVATION + year, 
         data = data)
E1 <- rstandard(M1)
...
# Step 2 - fit the gls
M2 <- gls(max_height ~ aspect_card + graze + sand + clay + ELEVATION + year, 
          method = "ML", data = data)
summary(M2)
AIC(M2) # 766.9835
...
M3 <- lmer(max_height ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
...
M3.full <- lmer(max_height ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop graze
M3.full <- lmer(max_height ~ aspect_card + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop elevation
M3.full <- lmer(max_height ~ aspect_card + sand + clay + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
