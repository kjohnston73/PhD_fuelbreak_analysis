...
M1 <- lm(herbload ~ aspect_card + graze + sand + clay + imazapic_binary + ELEVATION + year + precip + pdsi_prev_year + pdsi_samp_year, 
         data = ht_load)
E1 <- rstandard(M1)
...
# Step 2 - fit the gls
M2 <- gls(herbload ~ aspect_card + graze + sand + clay + imazapic_binary + ELEVATION + year + precip, 
          method = "ML", data =ht_load)
summary(M2)
AIC(M2) # 4068.025
...
M3 <- lmer(herbload ~ aspect_card + graze + sand + clay + imazapic_binary + ELEVATION + year + precip + (1 | plot), REML = TRUE, data = ht_load)
...
M3.full <- lmer(herbload ~ aspect_card + graze + sand + clay + imazapic_binary + ELEVATION + year + precip + (1 | plot), REML = TRUE, data = ht_load)
drop1(M3.full)
#drop aspect
M3.full <- lmer(herbload ~ graze + sand + clay + imazapic_binary + ELEVATION + year + precip  + (1 | plot), REML = TRUE, data = ht_load)
drop1(M3.full)
#drop imazapic
M3.full <- lmer(herbload ~ graze + sand + clay + ELEVATION + year + precip  + (1 | plot), REML = TRUE, data = ht_load)
drop1(M3.full)
#drop clay
M3.full <- lmer(herbload ~ graze + sand + ELEVATION + year + precip  + (1 | plot), REML = TRUE, data = ht_load)
drop1(M3.full)
#drop graze
M3.full <- lmer(herbload ~ sand + ELEVATION + year + precip + (1 | plot), REML = TRUE, data = ht_load)
drop1(M3.full)
