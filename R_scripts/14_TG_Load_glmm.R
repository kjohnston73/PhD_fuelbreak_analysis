...
data <- subset(ht_load, graze =='T' | graze == 'AT')

# GLMM process
# Step 1 - linear regression
M1 <- lm(herbload ~ aspect_card + graze + sand + clay + ELEVATION + year + precip + pdsi_prev_year + pdsi_samp_year, 
         data = data)
E1 <- rstandard(M1)
...
# Step 2 - fit the gls
M2 <- gls(herbload ~ aspect_card + graze + sand + clay + ELEVATION + year, 
          method = "ML", data = data)
summary(M2)
AIC(M2) # 4068.025
...
M3 <- lmer(herbload ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
...
M3.full <- lmer(herbload ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop aspect
M3.full <- lmer(herbload ~ graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop sand
M3.full <- lmer(herbload ~ graze + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop graze
M3.full <- lmer(herbload ~ clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop clay
M3.full <- lmer(herbload ~ ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
