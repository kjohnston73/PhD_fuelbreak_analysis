...
M1 <- lm(IAG ~ aspect_card + graze + sand + clay + ELEVATION + year + precip + pdsi_prev_year + pdsi_samp_year, 
         data = data)
E1 <- rstandard(M1)
...
# Step 2 - fit the gls
M2 <- gls(IAG ~ aspect_card + graze + sand + clay + ELEVATION + year, 
          method = "ML", data = data)
summary(M2)
AIC(M2) # 4068.025
...
M3 <- lmer(IAG ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
...
M3.full <- lmer(IAG ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop graze 
M3.full <- lmer(IAG ~ aspect_card + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop sand 
M3.full <- lmer(IAG ~ aspect_card + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
