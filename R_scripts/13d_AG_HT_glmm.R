...
M1 <- lm(max_height ~ aspect_card + graze + sand + clay + ELEVATION +
...
# Step 2 - fit the gls
M2 <- gls(max_height ~ aspect_card + graze + sand + clay + ELEVATION +
            imazapic_binary + year, 
          method = "ML", data = data)
summary(M2)
AIC(M2) # 766.9835
...
M3 <- lmer(max_height ~ aspect_card + graze + sand + clay + ELEVATION +
...
M3.full <- lmer(max_height ~ aspect_card + graze + sand + clay + ELEVATION +
                  imazapic_binary + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop aspect
M3.full <- lmer(max_height ~ graze + sand + clay + ELEVATION +
                  imazapic_binary + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop elevation
M3.full <- lmer(max_height ~ graze + sand + clay + imazapic_binary + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop imazapic
M3.full <- lmer(max_height ~ graze + sand + clay + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
