...
M1 <- lm(WEED ~ aspect_card + graze + sand + clay + ELEVATION + year, 
         data = data)
E1 <- rstandard(M1)
...
# Step 2 - fit the gls
M2 <- gls(WEED ~ aspect_card + graze + sand + clay + ELEVATION + year, 
          method = "ML", data = data)
summary(M2)
AIC(M2) # 669.2447
...
M3 <- lmer(WEED ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
...
M3.full <- lmer(WEED ~ aspect_card + graze + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop graze
M3.full <- lmer(WEED ~ aspect_card + sand + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
#drop sand 
M3.full <- lmer(WEED ~ aspect_card + clay + ELEVATION + year + (1 | plot), REML = TRUE, data = data)
drop1(M3.full)
